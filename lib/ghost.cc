...
int Ghost::SchedTaskEnterGhost(const Gtid& gtid, int dir_fd) {
  ...
  ret = SchedTaskEnterGhost(gtid.id(), dir_fd);
  absl::Time end = absl::Now();
  // std::ofstream outfile;
  // outfile.open("tests/log/output.txt", std::ios::app);
  // outfile << "SchedTaskEnterGhost " << gtid << " took "
  // << absl::ToDoubleMicroseconds(end - start) << " us" << std::endl;
  // outfile.close();
  return ret;
}
...
GhostThread::GhostThread(KernelScheduler ksched, std::function<void()> work,
  ...
  thread_ = std::thread([this, w = std::move(work), dir_fd] {
    ...
    if (ksched_ == KernelScheduler::kGhost) {
      const int ret = GhostHelper()->SchedTaskEnterGhost(/*pid=*/gtid_, dir_fd);
