...
struct CfsTask : public Task<> {
    ...
    bool new_to_cfs = true;
    bool has_run = false;
    void UpdateRuntime() {
        int ret =
            GhostHelper()->GetTaskRuntime(this->gtid, this->elapsed_runtime);
    }
...
class CfsRq {
   public:
    ...
    CfsTask* PickNextTask(CfsTask* prev, TaskAllocator<CfsTask>* allocator,
                          CfsCpuState* cs) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_);
    ...
    int DetachTasks(const CfsCpuState* dst_cs, int n,
    ...
    // `can_migrate_task`.
    bool CanMigrateTask(CfsTask* task, const CfsCpuState* dst_cs);
    ...
    // vruntime.
    void UpdateMinVruntime(CfsCpuState* cs) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_);

    void Show() const ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
        if (rq_.empty()) {
            std::cout << "Run queue is empty." << std::endl;
        } else {
            for (const auto& task : rq_) {
                std::cout << task->gtid.describe() << std::endl;
            }
        }
    }
...
class CfsGlobalRq {
   public:
    CfsGlobalRq() = default;
    CfsGlobalRq(const CfsGlobalRq&) = delete;
    CfsGlobalRq& operator=(CfsGlobalRq&) = delete;
    ...
    void RemoveFromRunqueue(CfsTask* task);

   private:
    mutable absl::Mutex mu_;
    std::deque<CfsTask*> run_queue_ ABSL_GUARDED_BY(mu_);
};
...
struct CfsCpuState {
    ...
    std::atomic<bool> ConvertToShort = false;
...
class CfsScheduler : public BasicDispatchScheduler<CfsTask> {
   ...
   private:
    ...
    CfsCpuState* cpu_state(const Cpu& cpu) { return &cpu_states_[cpu.id()]; }

    CfsCpuState* cpu_state_of(const CfsTask* task) {
        CHECK_GE(task->cpu, 0);
    ...
    CfsCpuState cpu_states_[MAX_CPUS];
...
class CfsAgent : public LocalAgent {
   public:
    CfsAgent(Enclave* enclave, Cpu cpu, CfsScheduler* scheduler)
        : LocalAgent(enclave, cpu), cfs_scheduler_(scheduler) {}

    void AgentThread() override;
    Scheduler* AgentScheduler() const override { return cfs_scheduler_; }
...
class FullCfsAgent : public FullAgent<EnclaveType> {
    ...
    void RpcHandler(int64_t req, const AgentRpcArgs& args,
        ...
        switch (req) {
            case CfsScheduler::kDebugRunqueue:
                cfs_scheduler_->debug_runqueue_ = true;
                response.response_code = 0;
                return;
            case CfsScheduler::kCountAllTasks:
                response.response_code = cfs_scheduler_->CountAllTasks();
                return;
            default:
   ...
   private:
    std::unique_ptr<CfsScheduler> cfs_scheduler_;
};
