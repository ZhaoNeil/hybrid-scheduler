...
void CfsScheduler::TaskNew(CfsTask* task, const Message& msg) {
    ...
    outfile.open("project/log/metrics_CFS_firecracker.txt", std::ios::app);
...
void CfsScheduler::CfsSchedule(const Cpu& cpu, BarrierToken agent_barrier,
    ...
    if (prio_boost) {
        ...
        // only sync up our CpuState in PickNextTask, we can simply RTLA yield.
        // This works because:
        // - We get prio_boosted
        // - We rtla yield
...
void CfsAgent::AgentThread() {
    ...
    // std::ofstream outfile;
    // outfile.open(
    //     "/home/yuxuan/ghost-userspace/project/log/preemt_count_cfs.txt",
    //     std::ios::app);
    // if (!outfile.is_open()) {
